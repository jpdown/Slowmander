...
export class LockdownPresets {
    ...
    public getPreset(
        guildId: Snowflake,
        preset: string
    ): LockdownPreset | null | undefined {
        try {
            const row: LockdownPreset = this.db
                .prepare(
    ...
    public removePreset(guildId: Snowflake, preset: string): boolean {
        ...
        try {
            ...
            info = this.db
                .prepare(
                    "DELETE FROM LockdownPresets WHERE guildId = ? AND preset = ?;"
                )
                .run(guildId, preset);
            rowsModified += info.changes;
