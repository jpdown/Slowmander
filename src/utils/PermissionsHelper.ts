...
export class PermissionsHelper {
    public static async checkPerms(
        ctx: CommandContext,
        command: Command
    ): Promise<boolean> {
    ...
    public static async getUserPermLevel(
        ctx: CommandContext
    ...
    public static async getMemberPermLevel(
        ctx: CommandContext
    ): Promise<PermissionLevel> {
        ...
        if (
            !channel
                ...
                .has(Permissions.FLAGS.USE_APPLICATION_COMMANDS)
        ) {
            return PermissionLevel.Disabled;
        }
        // We have slash command permission, continue
        if (
            (tempRole = ctx.bot.db.guildConfigs.getAdminRole(
                member.guild.id
            )) &&
        ...
        if (
            (tempRole = ctx.bot.db.guildConfigs.getModRole(member.guild.id)) &&
            roleList.has(tempRole)
        ) {
            return PermissionLevel.Mod;
        }
        if (
            (tempRole = ctx.bot.db.guildConfigs.getVipRole(member.guild.id)) &&
            roleList.has(tempRole)
        ) {
