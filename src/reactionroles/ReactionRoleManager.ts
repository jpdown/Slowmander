...
export class ReactionRoleManager {
  ...
  public async onReady() {
    const guildReactionRoles: Map<Snowflake, ReactionRoleObject[]> = await this.bot.configs.reactionRoleConfig.getAllReactionRoles();
    let currChannel: TextChannel | NewsChannel;
    let currMessage: Message;
    const iterableReactionRoles: IterableIterator<ReactionRoleObject[]> = guildReactionRoles.values();

    for (const currGuild of iterableReactionRoles) {
      for (const currReactionRole of currGuild) {
        try {
  ...
  private async checkUsers(message: Message, reactionRole: ReactionRoleObject) {
    // Find our reaction
    const reaction: MessageReaction | undefined = message.reactions.cache.find((r) => r.emoji.identifier === reactionRole.emoteID);
    ...
    const guildMembers: GuildMember[] = Array.from((await message.guild.members.fetch()).values());
