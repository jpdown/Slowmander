...
export class Config {
    ...
    public loadConfig(): ConfigObject {
        ...
        if (existsSync(this.CONFIG_PATH)) {
            try {
                ...
                this.logger.logSync(
                    LogLevel.ERROR,
                    "Error loading main config file.",
                    err
                );
    ...
    public get errorWebhook(): WebhookClient | null {
        if (
            !this.configObject.errorWebhookId ||
            !this.configObject.errorWebhookToken
        ) {
            return null;
        }

        return new WebhookClient({
            id: this.configObject.errorWebhookId,
            token: this.configObject.errorWebhookToken,
        });
    }
