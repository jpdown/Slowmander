...
export class VerificationManager {
    ...
    public async onGuildMemberAdd(member: GuildMember) {
        ...
        try {
            ...
            await ModErrorLog.log(
                `Unknown error applying verification role to ${member.user.tag}`,
                member.guild,
                this.bot
            );
            await this.logger.error(
                `Error adding verification role to user ${member.user.tag} in guild ${member.guild.name}.`,
                err
            );
        }
    }
    ...
    public async onMessageReactionAdd(
        ...
        if (fullReaction.emoji.identifier === config.emoteId) {
            ...
            try {
                ...
                await ModErrorLog.log(
                    `Unknown error removing verification role from ${member.user.tag}`,
                    member.guild,
                    this.bot
                );
                await this.logger.error(
                    `Error removing verification role from user ${member.user.tag} in guild ${member.guild.name}.`,
                    err
                );
            }
        }
        ...
        if (
            !fullMessage.channel
                .permissionsFor(fullMessage.guild.me)
        ...
        try {
            ...
            await ModErrorLog.log(
                `Unknown error removing verification reaction from ${member.user.tag}`,
                member.guild,
                this.bot
            );
            await this.logger.error(
                `Error removing verification reaction from user ${member.user.tag} in guild ${member.guild.name}.`,
                err
            );
        }
    }
    ...
    private async applyRole(
        ...
        if (!role) {
            await ModErrorLog.log(
                "Verification was enabled but role was unable to be found, disabling verification.",
                member.guild,
                this.bot
            );
        ...
        if (!member.guild.me.permissions.has(Permissions.FLAGS.MANAGE_ROLES)) {
            // If no manage roles
            await ModErrorLog.log(
                "Verification is enabled but I do not have the Manage Roles permission.",
                member.guild,
                this.bot
            );
            ...
            await ModErrorLog.log(
                "Verification is enabled but incorrect hierarchy, my top role is not above verification role.",
                member.guild,
                this.bot
            );
        }
